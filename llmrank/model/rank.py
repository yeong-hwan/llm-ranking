...
class Rank(SequentialRecommender):
    ...
    def load_interaction_data(self):
        ...
        # Load interaction data based on dataset_name
        inter_path = osp.join(self.data_path, f'{self.dataset_name}.inter')
        if self.dataset_name in ['ml-1m', 'ml-1m-full']:
            # Load ml-1m interaction data
            interaction_data = self.load_ml_1m_interaction_data(inter_path)
        elif self.dataset_name in ['Games', 'Games-6k']:
            # Load Games interaction data
            interaction_data = self.load_games_interaction_data(inter_path)
        else:
            raise NotImplementedError(f'Dataset {self.dataset_name} interaction data loading not implemented.')
